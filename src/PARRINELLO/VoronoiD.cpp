...
class VoronoiD : public Colvar {
  ...
  bool firsttime;
  int  lambda;
  int num_atomsa,num_atomsb,num_atoms;
  double d0, r0, sum_exp;

public:
  explicit VoronoiD(const ActionOptions&);
  ~VoronoiD();
// active methods:
...
PLUMED_REGISTER_ACTION(VoronoiD,"VORONOID")

void VoronoiD::registerKeywords( Keywords& keys ) {
...
VoronoiD::VoronoiD(const ActionOptions&ao):
...
void VoronoiD::calculate()
{
  ...
  #pragma omp parallel num_threads(nt)
  {
    ...
    //the local indexes of groupA here are at the start
    ...
    for(unsigned int j=0;j<num_atomsa;j++) {
      for(unsigned int k=j+1;k<num_atomsa;k++) {
        ...
        IonDistance -= buffer; //this is the s_d, the result!
    ...
    for(unsigned int i=0; i<nn; i+=1) {  //it is equivalent to the loop on n,j of Emanuele. The derivative will be evaulated on these two atoms
        for(unsigned int k=0;k<num_atomsa;k++) {
          ...
          if (ind0 == k) d_in = 1;
          if (ind0 != k) d_in = 0;
          fk = lambda * c[ind0][ind1] * (d_in - c[k][ind1]);

          for(unsigned int j=k+1;j<num_atomsa;j++) {
            if (ind0 == j) d_in = 1;
            if (ind0 != j) d_in = 0;
            fj = lambda * c[ind0][ind1] * (d_in - c[j][ind1]);
            buf=distGA[k][j]*(charge[k]*fj + charge[j]*fk);
  ...
  //local list, getAbsoluteIndexes()[k] gives the absolute index of every element which is different when one rebuilds the list
  //for(unsigned int k=0; k<getNumberOfAtoms(); k+=1) {
  //  printf("%d  %d %e \n",k,getAbsoluteIndexes()[k],nnexpnorm[k] );      
  //}
  //for(unsigned int k=0; k<getNumberOfAtoms(); k+=1) {
  //  printf("%d  %e  \n",getAbsoluteIndexes()[k],nnexpnorm[k]);          
  //}
  //for(unsigned int k=0; k<num_atomsa; k+=1) {
  //  printf("%d  %f \n",k,charge[k] );      
  //}  
  //printf("%d %d %d \n",nn,getNumberOfAtoms(),num_atoms); 
  //log.printf("\n\n\n\n");
