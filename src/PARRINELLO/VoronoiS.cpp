...
public:
  explicit VoronoiS(const ActionOptions&);
  ~VoronoiS();
// active methods:
...
PLUMED_REGISTER_ACTION(VoronoiS,"VORONOIS")

void VoronoiS::registerKeywords( Keywords& keys ) {
...
VoronoiS::VoronoiS(const ActionOptions&ao):
...
void VoronoiS::calculate()
{
  ...
  #pragma omp parallel num_threads(nt)
  {
    ...
    for(unsigned int j=0; j<groupasel; j+=1) {    
      charge[j]-=d0;
    ...
    for(unsigned int i=0; i<nn; i+=1) {  //it is equivalent to the loop on n,j of Emanuele. The derivative will be evaulated on these two atoms
        ...
        for(unsigned int k=0;k<groupasel;k++) {
          ...
          //if(pbc) {  //unfortunately I have to calculate the distance again, indexA k with indexB ind1. I don't know with i it corresponds
          //  distance_kind1=pbcDistance(getPosition(k),getPosition(ind1));
          //} else {
          //  distance_kind1=delta(getPosition(k),getPosition(ind1));
          //}
          //dinvmod=1.0/distABmod[k][ind1];
