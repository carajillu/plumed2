...
for i in $(seq $start $end); do
    ...
    if [ $i -gt 0 ]; then
       rm prod.tpr
       sed -i "s/NPROBES=16 PROBESTRIDE=25000/NPROBES=16 PROBESTRIDE=25000 RESTART_PROBES/g" plumed.dat
       let a=i-1
    ...
    $gmx mdrun -v -ntomp 8 -pin on -deffnm prod -plumed plumed.dat -nsteps 25000000
    cd $rootdir 	
done
