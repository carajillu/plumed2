...
void Com::init(unsigned N_ligand, vector<double> Masses_ligand)
{
 n_ligand=N_ligand;
...
void Com::calculate_com(vector<vector<double>> ligand_xyz)
{
 xyz[0]=0;
 xyz[1]=0;
 xyz[2]=0;
 ...
 for (unsigned j=0;j<n_ligand;j++)
 {
  xyz[0]+=ligand_xyz[j][0]*masses_ligand[j];
  xyz[1]+=ligand_xyz[j][1]*masses_ligand[j];
  xyz[2]+=ligand_xyz[j][2]*masses_ligand[j];
 }
 xyz[0]/=total_mass;
 xyz[1]/=total_mass;
 xyz[2]/=total_mass;
